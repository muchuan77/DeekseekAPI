<template>
  <div class="flow-manager">
    <el-steps :active="currentStep" finish-status="success" class="flow-steps">
      <el-step title="AI分析" description="输入并分析内容" />
      <el-step title="创建谣言" description="创建谣言记录" />
      <el-step title="传播分析" description="分析传播路径" />
      <el-step title="查看结果" description="查看完整分析" />
    </el-steps>
    ...
    <div v-if="currentStep === 1" class="step-content">
      <el-card class="rumor-card">
        ...
        <el-form :model="rumorForm" label-width="100px">
          <el-form-item label="标题">
            <el-input v-model="rumorForm.title" />
          </el-form-item>
          <el-form-item label="内容">
            <el-input v-model="rumorForm.content" type="textarea" :rows="4" />
          </el-form-item>
          <el-form-item label="来源">
            <el-input v-model="rumorForm.source" />
          </el-form-item>
          <el-form-item>
            <el-button type="primary" @click="createRumor">创建谣言</el-button>
          </el-form-item>
        </el-form>
      </el-card>
    </div>
    ...
    <div v-if="currentStep === 2" class="step-content">
      <Propagation :rumor-id="currentRumorId" @analysis-complete="handlePropagationComplete" />
    </div>
